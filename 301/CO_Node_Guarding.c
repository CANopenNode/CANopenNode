...
CO_nodeGuardingSlave_init(CO_nodeGuardingSlave_t* ngs, OD_entry_t* OD_100C_GuardTime,
    ...
    ret = CO_CANrxBufferInit(CANdevRx, CANdevRxIdx, CANidNodeGuarding & CO_COB_STD_MASK, CO_COB_STD_MASK, true, (void*)ngs, CO_ngs_receive);
    if (ret != CO_ERROR_NO) {
        return ret;
    }
...
CO_nodeGuardingMaster_init(CO_nodeGuardingMaster_t* ngm, CO_EM_t* em, CO_CANmodule_t* CANdevRx, uint16_t CANdevRxIdx,
    ...
    ret = CO_CANrxBufferInit(CANdevRx, CANdevRxIdx, CO_CAN_ID_HEARTBEAT, CO_COB_STD_MASK ^ 0x7FU, false, (void*)ngm, CO_ngm_receive);
    if (ret != CO_ERROR_NO) {
        return ret;
    }
