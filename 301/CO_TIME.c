...
CO_TIME_init(CO_TIME_t* TIME, OD_entry_t* OD_1012_cobIdTimeStamp, CO_CANmodule_t* CANdevRx, uint16_t CANdevRxIdx,
    ...
    CO_CANident_t cobId = (CO_CANident_t)(cobIdTimeStamp & COB_CAN_ID_MASK);
    TIME->isConsumer = (cobIdTimeStamp & 0x80000000UL) != 0U;
    TIME->isProducer = (cobIdTimeStamp & 0x40000000UL) != 0U;
    ...
    if (TIME->isConsumer) {
        CO_ReturnError_t ret = CO_CANrxBufferInit(CANdevRx, CANdevRxIdx, cobId, COB_CAN_ID_MASK, false, (void*)TIME,
                                                  CO_TIME_receive);
        if (ret != CO_ERROR_NO) {
            return ret;
        }
